...
void * aloca_reg(float embedding[], char id[]){
    ...
    memcpy(reg->embedding, embedding, sizeof(float) * EMBEDDING_SIZE);
    strncpy(reg->id, id, 100);
    return reg;
}


int comparador(void *a, void *b, int pos){
    float ea = ((treg *)a)->embedding[pos];
    float eb = ((treg *)b)->embedding[pos];
    if (ea < eb) return -1;
